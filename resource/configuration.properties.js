 {
  ...
  ALL: {
	...
	attivioProtocol:'http',
	attivioHostname:'47.103.133.15',//这是attivio服务所在ip地址，不要写成127.0.0.1要写成实际地址如：47.103.133.15
	attivioPort:'17000',//这是attivio服务开放端口
	fieldValue:'sourcepath',//这是export导出的字段
	downLoadParams: {//searchui下载功能参数，这个针对性很强与业务相关
			id:'住院门诊号_s',//理论上所有field都可以，但这里推荐找一个能唯一标识患者的id
			field:'病历类型_s',//理论下所有field都可以，但这里推荐找一个能唯一标识患者的病历类型
			defauleValue:'病历内容_s',//所有field都可以，但这里推荐找一个表示这个患者你正在关心的内容
			limitLength:10000,//一次下载量做一个限制
			downValueLength:10//匹配到的内容如果很长，就只会匹配关键信息前后10个长度的大小
	},
